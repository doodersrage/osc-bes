...
// added by splautz for surls
  if (isset($HTTP_GET_VARS['page']) && tep_not_null($HTTP_GET_VARS['page'])) {
    $HTTP_GET_VARS['reviews_id'] = $HTTP_GET_VARS['page'];
...
// changed by splautz for surls
//    tep_redirect(tep_href_link(FILENAME_PRODUCT_REVIEWS, tep_get_all_get_params(array('reviews_id'))));
      tep_redirect(tep_href_link(FILENAME_PRODUCT_REVIEWS, tep_get_all_get_params(array('reviews_id','page'))));
    }
  } else {
// changed by spautz for surls
//  tep_redirect(tep_href_link(FILENAME_PRODUCT_REVIEWS, tep_get_all_get_params(array('reviews_id'))));
    tep_redirect(tep_href_link(FILENAME_PRODUCT_REVIEWS, tep_get_all_get_params(array('reviews_id','page'))));
  }
...
// added by splautz to append review number to title
  if (defined('HEAD_TITLE_REVIEW') && strlen(HEAD_TITLE_REVIEW)) $head_title_page = sprintf(HEAD_TITLE_REVIEW, $HTTP_GET_VARS['reviews_id']);

  $breadcrumb->add(NAVBAR_TITLE, tep_href_link(FILENAME_PRODUCT_REVIEWS, tep_get_all_get_params(array('reviews_id'))));
?>
    ...
    <td width="100%" valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="0">
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            <td class="pageHeading" valign="top"><?php echo '<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $review['products_id']) . '"><h2>' . $products_name . '</h2></a>'; ?></td>
...
<?php
  if (tep_not_null($review['products_image']) && $imagedata = getimagesize(DIR_WS_IMAGES . $review['products_image'])) {
?>
<script language="javascript"><!--
document.write('<?php echo '<a href="javascript:popupWindow(\\\'' . tep_href_link(FILENAME_POPUP_IMAGE, 'pID=' . $review['products_id']) . '\\\'' . ',' . ($imagedata[0]+20) . ',' . ($imagedata[1]+20) . ')">' . tep_image(DIR_WS_IMAGES . $review['products_image'], addslashes($review['products_img_alt']?$review['products_img_alt']:$review['products_name']), SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'hspace="5" vspace="5"') . '<br>' . tep_image_button('image_enlarge.gif', TEXT_CLICK_TO_ENLARGE) . '</a>'; ?>');
//--></script>
<noscript>
<?php echo '<a href="' . tep_href_link(DIR_WS_IMAGES . $review['products_image']) . '" target="_blank">' . tep_image(DIR_WS_IMAGES . $review['products_image'], $review['products_img_alt']?$review['products_img_alt']:$review['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'hspace="5" vspace="5"') . '<br>' . tep_image_button('image_enlarge.gif', TEXT_CLICK_TO_ENLARGE) . '</a>'; ?>
</noscript>
<?php
  }

  echo '<p><a href="' . tep_href_link(basename($PHP_SELF), tep_get_all_get_params(array('action')) . 'action=buy_now' . (!empty($review['products_affiliate_url'])?'_aff':'')) . '">' . (empty($review['products_affiliate_url'])?tep_image_button('button_in_cart.gif', IMAGE_BUTTON_IN_CART):tep_image_button('affiliate_buy_now.gif', IMAGE_BUTTON_BUY_NOW)) . '</a></p>';
?>
                </td>
              </tr>
            </table>
