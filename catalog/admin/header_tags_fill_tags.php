...
*/
  ...
  if (isset($products_fill))
  {
    ...
    { 
      $products_tags_query = tep_db_query("select products_name, products_description, products_info, products_id, products_head_title_tag, products_head_desc_tag, products_head_keywords_tag, language_id from " . TABLE_PRODUCTS_DESCRIPTION . " where language_id = '" . $langID . "'");
      while ($products_tags = tep_db_fetch_array($products_tags_query))
      {
        ...
        if ($updateDB)
        {
          ...
          {                                                     //product description
            if (strlen($products_tags['products_info'].$products_tags['products_description']))
            {
              $desc = strip_tags($products_tags['products_info'].' '.$products_tags['products_description']);
              if (isset($productsMetaDescLength) && (int)$productsMetaDescLength > 3 && (int)$productsMetaDescLength < strlen($desc))
                $desc = substr($desc, 0, (int)$productsMetaDescLength);
            }   
            else
          ...
          tep_db_query("update " . TABLE_PRODUCTS_DESCRIPTION . " set products_head_title_tag='".addslashes($products_tags['products_name'])."', products_head_desc_tag = '". addslashes($desc)."', products_head_keywords_tag =  '" . addslashes($products_tags['products_name']) . "' where products_id = '" . $products_tags['products_id'] . "' and language_id='". $langID ."'");
        } 
      }  
    }
  }
