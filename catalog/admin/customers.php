...
// modified by splautz to fix disconnected primary address book entries.
// ########## Ajout/edite commande et compte client ##########
// $customersrecords = mysql_query("SELECT * FROM customers") or die ("What Happen??? Error 1");
    $customersrecords = tep_db_query("select c.customers_id, a.customers_id as a_customers_id from " . TABLE_CUSTOMERS . " c left join " . TABLE_ADDRESS_BOOK . " a on c.customers_id = a.customers_id and c.customers_default_address_id = a.address_book_id where a.address_book_id is null");
    while($customerrows = tep_db_fetch_array($customersrecords))
    {
      $e = tep_db_query("SELECT address_book_id FROM address_book WHERE customers_id ='".$customerrows['customers_id']."'") or die ("What Happen??? Error 2");
      $real = tep_db_fetch_array($e);
      $updatedefaultaddress = tep_db_query("UPDATE customers SET customers_default_address_id = '".$real['address_book_id']."' WHERE customers_id='".$customerrows['customers_id']."'") or die ("What Happen??? Error 3");
    }
      ...
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
          ...
          switch ($listing) {
              ...
              break;
/* modified by splautz for company sort
              case "firstname":
...
*/
              case "company":
              $order = "a.entry_company, c.customers_id";
              break;
              case "company-desc":
              $order = "a.entry_company DESC, c.customers_id";
              break;
              case "name":
              $order = "c.customers_lastname, c.customers_firstname, c.customers_id";
              break;
              case "name-desc":
              $order = "c.customers_lastname DESC, c.customers_firstname DESC, c.customers_id";
              break;
...
<?php /* modified by splautz for company sorting
...
*/ ?>
                <td class="dataTableHeadingContent"><a href="<?php echo "$PHP_SELF?listing=name"; ?>"><?php echo tep_image_button('ic_up.gif', ' Sort ' . TABLE_HEADING_NAME . ' --> A-B-C From Top '); ?></a>&nbsp;<a href="<?php echo "$PHP_SELF?listing=name-desc"; ?>"><?php echo tep_image_button('ic_down.gif', ' Sort ' . TABLE_HEADING_NAME . ' --> Z-X-Y From Top '); ?></a><br><?php echo TABLE_HEADING_NAME; ?></td>
                <td class="dataTableHeadingContent"><a href="<?php echo "$PHP_SELF?listing=company"; ?>"><?php echo tep_image_button('ic_up.gif', ' Sort ' . TABLE_HEADING_COMPANY . ' --> A-B-C From Top '); ?></a>&nbsp;<a href="<?php echo "$PHP_SELF?listing=company-desc"; ?>"><?php echo tep_image_button('ic_down.gif', ' Sort ' . TABLE_HEADING_COMPANY . ' --> Z-X-Y From Top '); ?></a><br><?php echo TABLE_HEADING_COMPANY; ?></td>
...
// modified by splautz for company searches
...
?>
                <td class="dataTableContent"><?php echo $customers['customers_lastname'] . ', ' . $customers['customers_firstname']; ?></td>
  ...
  switch ($action) {
    ...
    default:
      if (isset($cInfo) && is_object($cInfo)) {
        ...
        $contents[] = array('align' => 'center', 'text' => '<a href="' . HTTP_CATALOG_SERVER . DIR_WS_CATALOG . 'password_forgotten.php?cID=' . $cInfo->customers_id . '&email_address=' . $cInfo->customers_email_address . '">' . TEXT_SEND_PASSWORD . '</a>');  // added by splautz to send new password
        $contents[] = array('text' => '<br>' . TEXT_EMAIL_ADDRESS . ' ' . $cInfo->customers_email_address);  // added by splautz to display email
