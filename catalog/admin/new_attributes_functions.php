...
function checkAttribute( $current_value_id, $current_product_id, $current_product_option_id )
{
 ...
 if ($isFound) {
    ...
    while($line = mysql_fetch_array($result, MYSQL_ASSOC)) {
        ...
        if ($attribute_value_prefix == "+") { $zeroCheck = "";
                                              $posCheck = " SELECTED";
                                              $negCheck = "";
        } elseif ($attribute_value_prefix == "-") { $zeroCheck = "";
                                              $posCheck = "";
                                              $negCheck = " SELECTED";
        } else { $zeroCheck = " SELECTED";
                 $posCheck = "";
                 $negCheck = "";
        }
...
function getSortCopyValues( $current_value_id, $current_product_id )
{
 ...
 if ( $isSelected ) {
        ...
        while ($line = mysql_fetch_array($result, MYSQL_ASSOC)) {
                ...
                $attribute_sort = $line['products_attributes_sort_order'];
...
//        	$attribute_sort = "0";  // removed by splautz to default sort to that in products_options_values_to_products_options table
//        	$attribute_weight = "";
