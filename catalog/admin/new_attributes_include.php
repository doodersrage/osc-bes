...
$query = "SELECT * FROM products_options where products_options_id LIKE '%' AND language_id = '$languageFilter' ORDER BY COALESCE(products_options_sort_order,10000), products_options_id";
...
// Find all of the Current Option's Available Values
        $query2 = "SELECT * FROM products_options_values_to_products_options WHERE products_options_id = '$current_product_option_id' ORDER BY COALESCE(products_options_values_sort_order,10000), products_options_values_id";
        ...
        if ($matches2) {
           ...
           while ($line = mysql_fetch_array($result2, MYSQL_ASSOC)) {
                ...
                if ( $optionSortCopyInstalled == "1" ) $attribute_sort = $line['products_options_values_sort_order'];  // added by splautz to default sort to that in products_options_values_to_products_options table
...
// Add Support for multiple text input option types (for Chandra's contribution).. and using ' to begin/end strings.. less of a mess.
                        ...
                        if ( $optionTypeInstalled == "1" ) {
                                ...
                                echo "<TD class=\"main\" align=\"left\"><SELECT name=\"" . $current_value_id . "_prefix\"><OPTION value=\"+\"" . $posCheck . ">+<OPTION value=\"-\"" . $negCheck . ">-<OPTION value=\" \"" . $zeroCheck . "> </SELECT></TD>";
