  ...
  if (tep_not_null($action)) {
    switch ($action) {
      ...
      case 'update':
        ...
        $homepage_only = tep_db_prepare_input($HTTP_POST_VARS['homepage_only']);
...
<?php
  if ($action == 'new') {
    ...
    if (isset($HTTP_GET_VARS['bID'])) {
      ...
      $banner_query = tep_db_query("select banners_title, banners_url, banners_image, banners_group, sort_order, homepage_only, banners_html_text, status, date_format(date_scheduled, '%d/%m/%Y') as date_scheduled, date_format(expires_date, '%d/%m/%Y') as expires_date, expires_impressions, date_status_change from " . TABLE_BANNERS . " where banners_id = '" . (int)$bID . "'");
      $banner = tep_db_fetch_array($banner_query);
  ...
  var dateScheduled = new ctlSpiffyCalendarBox("dateScheduled", "new_banner", "date_scheduled","btnDate2","<?php echo $bInfo->date_scheduled; ?>",scBTNMODE_CUSTOMBLUE);
      ...
      <tr><?php echo tep_draw_form('new_banner', FILENAME_BANNER_MANAGER, (isset($HTTP_GET_VARS['page']) ? 'page=' . $HTTP_GET_VARS['page'] . '&' : '') . 'action=' . $form_action, 'post', 'enctype="multipart/form-data"'); if ($form_action == 'update') echo tep_draw_hidden_field('banners_id', $bID); ?>
        <td><table border="0" cellspacing="0" cellpadding="2">
          ...
          <tr>
            <td class="main" valign="top"><?php echo TEXT_SORT_ORDER; ?></td>
            <td class="main"><?php echo tep_draw_input_field('sort_order', $bInfo->sort_order, 'size="3" maxlength="3"'); ?></td>
          </tr>
          <tr>
            <td class="main" valign="top"><?php echo TEXT_HOMEPAGE_ONLY; ?></td>
            <td class="main"><?php echo tep_draw_checkbox_field('homepage_only', '1', $bInfo->homepage_only); ?></td>
          </tr>
          <tr>
            <td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
          </tr>
          ...
          <tr>
            <td valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr class="dataTableHeadingRow">
                ...
                <td class="dataTableHeadingContent" align="center"><?php echo TABLE_HEADING_HOMEPAGE_ONLY; ?></td>
                <td class="dataTableHeadingContent" align="right"><?php echo TABLE_HEADING_STATISTICS; ?></td>
                <td class="dataTableHeadingContent" align="right"><?php echo TABLE_HEADING_STATUS; ?></td>
                <td class="dataTableHeadingContent" align="right"><?php echo TABLE_HEADING_ACTION; ?>&nbsp;</td>
              </tr>
<?php
    $banners_query_raw = "select banners_id, banners_title, banners_image, banners_group, sort_order, homepage_only, status, expires_date, expires_impressions, date_status_change, date_scheduled, date_added from " . TABLE_BANNERS . " order by banners_group, COALESCE(sort_order,1000), banners_title";
...
?>
                ...
                <td class="dataTableContent" align="center"><?php echo $banners['sort_order']; ?></td>
                <td class="dataTableContent" align="center"><?php echo ($banners['homepage_only'] ? 'yes' : 'no'); ?></td>
