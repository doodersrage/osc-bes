...
function doCreate($folder) {
   ...
   if(!(file_exists(IMAGE_DIR . $path))) {
      ...
      else @chmod(IMAGE_DIR . $path, 0777);  // added by splautz to ensure permissions are set
   }
            ...
            if(!(file_exists(IMAGE_DIR . $path))) {
               ...
               if(UPLOAD_LIMIT == 0 || $size <= UPLOAD_LIMIT) {
                  ...
                  if(!(@copy($temp, (IMAGE_DIR . $path))))
                     ...
                     // ... report the error
                     $error = "File \'" . $path . "\' could not be created. To allow proper upload access, try creating sub-folders using FTP and set full permissions (777).";
