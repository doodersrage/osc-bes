<?php
include('includes/application_top_catalog.php');
//  Start TIMER
//  -----------
...
$OutFile = DIR_FS_CATALOG."feeds/".FROOGLE_FTP_FILENAME; 
...
if($convertCur)
{
$productURL = 'currency=' . $curType . '&' . $productURL;  //where CURTYPE is your currency type (eg. USD, EUR, GBP)
}
...
concat(products_description.products_info,' ',products_description.products_description) AS description,
...
specials.expires_date AS expDate
FROM
products
...
AND categories.categories_status != 0
ORDER BY
...
$output = "link \t title \t description \t price \t image_link \t category \t id \t expiration_date ";
...
while( $row = mysql_fetch_object( $result ) )
{
	...
	if( $row->prodStatus == 1 || ($optional_sec == 1 && $instock == 1) )
	{
		...
		$expDate = explode(' ', $row->expDate);

		$output .= tep_href_link(FILENAME_PRODUCT_INFO, $row->product_url) . "\t" .
...
$fout = fwrite( $fp , $output );
fclose( $fp );
echo "File generated: <a href=\"" . HTTP_SERVER.DIR_WS_CATALOG.'feeds/'.$destination_file . "\" target=\"_blank\">" . $destination_file . "</a>&nbsp;&nbsp;(Right click & select \"Save Target As...\" to download.)<br><br>\n";
@chmod($OutFile, 0777);
...
$conn_id = false;
if (function_exists('ftp_connect')) $conn_id = @ftp_connect($ftpservername);
if ( $conn_id == false )
{
