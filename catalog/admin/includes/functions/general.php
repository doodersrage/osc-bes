...
// Redirect to another page or site
  function tep_redirect($url) {
    global $surls_updated, $messageStack; // added by splautz
    global $logger;

  // added by splautz for running external script after surls updates
    tep_surls_update(true);
...
//    header("HTTP/1.x 301 Moved Permanently");  // added by splautz for 301 redirects
    header('Location: ' . $url);
...
// NOTE: Includes a workaround for dates before 01/01/1970 that fail on windows servers
  ...
  function tep_get_category_tree($parent_id = '0', $spacing = '', $exclude = '', $category_tree_array = '', $include_itself = false) {
    ...
    $categories_query = tep_db_query("select c.categories_id, cd.categories_name, c.parent_id from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.categories_id = cd.categories_id and cd.language_id = '" . (int)$languages_id . "' and c.parent_id = '" . (int)$parent_id . "' order by COALESCE(c.sort_order,1000), cd.categories_name");
...
// Text Mode
  ...
  function tep_get_category_surls_name($category_id, $language_id) {
    $category_query = tep_db_query("select su.surls_name from " . TABLE_CATEGORIES_DESCRIPTION . " cd, " . TABLE_SEO_URLS . " su where cd.categories_surls_id = su.surls_id and cd.categories_id = '" . (int)$category_id . "' and cd.language_id = '" . (int)$language_id . "'");
    if (tep_db_num_rows($category_query) == 1) {
      $category = tep_db_fetch_array($category_query);
      return $category['surls_name'];
    } else return '';
  }
  ...
  function tep_get_products_affiliate_url($product_id, $language_id) {
    $product_query = tep_db_query("select products_affiliate_url from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . (int)$product_id . "' and language_id = '" . (int)$language_id . "'");
    $product = tep_db_fetch_array($product_query);

    return $product['products_affiliate_url'];
  }

  function tep_get_products_surls_name($product_id, $language_id) {
    $product_query = tep_db_query("select su.surls_name from " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_SEO_URLS . " su where pd.products_surls_id = su.surls_id and pd.products_id = '" . (int)$product_id . "' and pd.language_id = '" . (int)$language_id . "'");
    if (tep_db_num_rows($product_query) == 1) {
      $product = tep_db_fetch_array($product_query);
      return $product['surls_name'];
    } else return '';
  }

  function tep_get_products_h1($product_id, $language_id) {
    $product_query = tep_db_query("select products_h1 from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . (int)$product_id . "' and language_id = '" . (int)$language_id . "'");
    $product = tep_db_fetch_array($product_query);

    return $product['products_h1'];
  }

  function tep_get_products_img_alt($product_id, $language_id) {
    $product_query = tep_db_query("select products_img_alt from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . (int)$product_id . "' and language_id = '" . (int)$language_id . "'");
    $product = tep_db_fetch_array($product_query);
...
// added by splautz for seo urls
  function tep_get_manufacturer_surls_name($manufacturer_id, $language_id) {
    $manufacturer_query = tep_db_query("select su.surls_name from " . TABLE_MANUFACTURERS_INFO . " mi, " . TABLE_SEO_URLS . " su where mi.manufacturers_surls_id = su.surls_id and mi.manufacturers_id = '" . (int)$manufacturer_id . "' and mi.languages_id = '" . (int)$language_id . "'");
    if (tep_db_num_rows($manufacturer_query) == 1) {
      $manufacturer = tep_db_fetch_array($manufacturer_query);
      return $manufacturer['surls_name'];
    } else return '';
  }
...
// Cant be used in safe mode.
  function tep_set_time_limit($limit) {
    if (!get_cfg_var('safe_mode')) {
	  @set_time_limit($limit);  // modified by splautz since the check didn't always work even though in safe mode
    }
  }
...
// added by splautz to remove an image along with all of its cached sizes
  function tep_remove_image($id,$group='p',$newimage='',$imagetable=false) {
    $groups = array('p' => 'products','c' => 'categories','m' => 'manufacturers','g' => 'pages');
    if (!$id || !isset($groups[$group])) return;
    $groupname = $groups[$group];
    if ($imagetable) $sourcename = 'images';
    else $sourcename = $groupname;
    $images_query = tep_db_query("select ".$sourcename."_image from " . constant('TABLE_'.strtoupper($sourcename)) . " where ".$sourcename."_id = '" . (int)$id . "'");
    $images = tep_db_fetch_array($images_query);
    $image = $images[$sourcename.'_image'];
    if (tep_not_null($image) && $image <> $newimage) {
      $duplicate_images_query = tep_db_query("select count(*) as total from " . constant('TABLE_'.strtoupper($groupname)) . " where ".$groupname."_image = '" . tep_db_input($image) . "'");
      $duplicate_images = tep_db_fetch_array($duplicate_images_query);
      $total = $duplicate_images['total'];
      $duplicate_images_query = tep_db_query("select count(*) as total from " . TABLE_IMAGES . " i where i.images_image = '" . tep_db_input($image) . "' and i.group_type = '" . $group . "'");
      $duplicate_images = tep_db_fetch_array($duplicate_images_query);
      $total += $duplicate_images['total'];
      if ($total < 2 && file_exists(DIR_FS_CATALOG_IMAGES . $image)) {
        @rm(DIR_FS_CATALOG_IMAGES . $image);
        $bimage = explode('.', $image);
        array_splice($bimage,count($bimage)-1,0,'*');
        @rm(DIR_FS_CATALOG_IMAGES . DIRNAME_IMAGECACHE . implode('.',$bimage));
      }
    }
  }

// added by splautz to remove all images for a group_id
  function tep_remove_all_images($id,$group='p') {
    $images_query = tep_db_query("select i.images_id from " . TABLE_IMAGES . " i where i.group_id = '" . (int)$id . "' and i.group_type = '" . $group . "'");
    while ($images = tep_db_fetch_array($images_query)) {
      $images_id = $images['images_id'];
      tep_remove_image($images_id,$group,'',true);
    ...
    tep_remove_image($id,$group);
  }

  function tep_remove_category($category_id) {
    global $surls_updated; // added by splautz

    // added by splautz to delete image
    tep_remove_all_images($category_id,'c');

    // added by splautz to delete surl entries
    $categories_query = tep_db_query("select cd.categories_surls_id from " . TABLE_CATEGORIES_DESCRIPTION . " cd where cd.categories_id = '" . (int)$category_id . "' and cd.categories_surls_id > 0");
    while($categories = tep_db_fetch_array($categories_query)) {
      tep_remove_surl($categories['categories_surls_id']);
      $surls_updated = true;
    }
  ...
  function tep_remove_product($product_id) {
    ...
    // added by splautz to delete image
    tep_remove_all_images($product_id,'p');

    // added by splautz to delete surl entries
    $products_query = tep_db_query("select pd.products_surls_id from " . TABLE_PRODUCTS_DESCRIPTION . " pd where pd.products_id = '" . (int)$product_id . "' and pd.products_surls_id > 0");
    while($products = tep_db_fetch_array($products_query)) {
      tep_remove_surl($products['products_surls_id']);
      $surls_updated = true;
    }
    ...
    tep_db_query("delete from " . TABLE_CUSTOMERS_BASKET . " where products_id = '" . (int)$product_id . "' or products_id like '" . (int)$product_id . "{%'");  // modified by splautz to clear all product variants (update-20060817)
    tep_db_query("delete from " . TABLE_CUSTOMERS_BASKET_ATTRIBUTES . " where products_id = '" . (int)$product_id . "' or products_id like '" . (int)$product_id . "{%'");  // modified by splautz to clear all product variants (update-20060817)
    tep_db_query("delete from " . TABLE_PRODUCTS_OPTIONS_PRODUCTS . " where pop_products_id_master = '" . (int)$product_id . "' or  pop_products_id_slave = '" . (int)$product_id . "'");  // added by splautz for related products
...
// END: Extra Fields Contribution
  ...
  // added by splautz to delete surl entry by id
  function tep_remove_surl($surls_id) {
    tep_db_query("delete from " . TABLE_SEO_URLS . " where surls_id = '" . (int)$surls_id . "'");
  }

  // added by splautz to delete surl entry by script name
  function tep_remove_script_surl($surls_script, $language_id) {
    tep_db_query("delete from " . TABLE_SEO_URLS . " where surls_script = '" . $surls_script . "' and surls_param = '' and language_id = '" . (int)$language_id . "'");
  }

  // added by splautz to get surl entry by script name
  function tep_get_script_surls_name($surls_script, $language_id, &$surls_id) {
    $surls_id = NULL;
    $surls_query = tep_db_query("select su.surls_id, su.surls_name from " . TABLE_SEO_URLS . " su where su.surls_script = '" . $surls_script . "' and su.surls_param = '' and su.language_id = '" . (int)$language_id . "'");
    if (tep_db_num_rows($surls_query) == 1) {
      $surls = tep_db_fetch_array($surls_query);
      $surls_id = $surls['surls_id'];
      return $surls['surls_name'];
    } else return '';
  }

  // added by splautz to check for dup surls
  function tep_check_dup_surl($session_warn, $exclude_id, $surls_name, $surls_script='', $surls_param='', $language_id='') {
    global $messageStack;
    $surls_query_str = "surls_name = '$surls_name'";
    if ($surls_script) $surls_query_str .= " or (surls_script = '$surls_script' and surls_param = '$surls_param' and language_id = '$language_id')";
    if (is_numeric($exclude_id)) $surls_query_str = "($surls_query_str and surls_id != $exclude_id)";
    $surls_query_str = "select surls_id from " . TABLE_SEO_URLS . " where $surls_query_str";
    $surls_query = tep_db_query($surls_query_str);
    if (tep_db_num_rows($surls_query) > 0) {
      $error = 'Error saving Url Name (Entry already exists) -> ' . $surls_name;
	  if ($surls_script) $error .= ' / ' . $surls_script . ' : ' . $surls_param . ' : ' . $language_id;
      if ($session_warn) $messageStack->add_session($error, 'warning');
      else $messageStack->add($error, 'warning');
	  return true;  // dup exists
    }
    return false; // dup not found
  }

  // added by splautz for running external script after surls updates
  function tep_surls_update($session_warn) {
    global $surls_updated, $messageStack;
//
//    if (!empty($surls_updated) && SURLS_SCRIPT) {
//      $start = time();
//      if (SURLS_CURL == 'FOPEN') {
//        if ($script=@fopen(SURLS_SCRIPT,'r')) {
//          stream_set_timeout($script, SURLS_TIMEOUT);
//	      $start = time();
//          while (!feof($script) && time()-$start < SURLS_TIMEOUT) fread($script, 1024);
//          if (!feof($script)) $error = 'Time-out running external surls script -> ' . SURLS_SCRIPT;
//          fclose($script);
//        } else $error = 'Unable to run external surls script -> ' . SURLS_SCRIPT;
//      } else {
//        if (SURLS_CURL == 'CURL') {
//          $ch = curl_init();
//          curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
//          curl_setopt($ch, CURLOPT_URL, SURLS_SCRIPT);
//          if (SURLS_TIMEOUT != 'NONE') curl_setopt($ch, CURLOPT_TIMEOUT, SURLS_TIMEOUT);
//          curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
//          curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
//          curl_setopt($ch, CURLOPT_HTTPHEADER, array("Referer: " . 'SURLS Update Script : ' . STORE_NAME));
//          $reply = curl_exec($ch);
//          curl_close($ch);
//        } else {
//          $command_line = SURLS_CURL . " " . (SURLS_TIMEOUT == 'NONE' ? '' : '-m ' . SURLS_TIMEOUT) . " -s -e 'SURLS Update Script : " . STORE_NAME . "' --url " . SURLS_SCRIPT . " '" . SURLS_SCRIPT . "'";
//          exec($command_line, $reply);
//          $reply = $reply[0];
//        }
//        if (time()-$start >= SURLS_TIMEOUT) $error = 'Time-out running external surls script -> ' . SURLS_SCRIPT;
//      } 
//      if (!empty($error))
//        if ($session_warn) $messageStack->add_session($error, 'warning');
//        else $messageStack->add($error, 'warning');
...
// added for Admin Only comments
    tep_db_query("delete from " . TABLE_ADMIN_COMMENTS . " where orders_id = '" . (int)$order_id . "'");
  }
...
// modified by splautz to support separate html & text msgs
//   - include $text to have a separate html & text version.  If $text is false or '', it will be built from html.
//   - leave $text as null to maintain standard OsC compatibility, i.e. lf's are replaced with <br>'s in html.

  function tep_mail($to_name, $to_email_address, $email_subject, $email_text, $from_email_name, $from_email_address, $text = NULL) {
...
// added by splautz to support separate html & text msgs
    if ($text !== NULL) {
      if (!tep_not_null($text)) {
        $text = preg_replace(array('/<head[^>]*>.*<\/head>/is'), array(''), $email_text);
        $text = strip_tags($text);
        $text = decode_entities($text, ENT_QUOTES);
      }
      if (EMAIL_USE_HTML == 'true') {
        $message->html = $email_text;
        $message->html_text = tep_convert_linefeeds(array("\r\n", "\n", "\r"), $message->lf, $text);
      } else $message->add_text($text);
    } else {

      // Build the text version
      $text = preg_replace(array('/<head[^>]*>.*<\/head>/is'), array(''), $email_text);  // added by splautz
      $text = strip_tags($text);
      $text = decode_entities($text, ENT_QUOTES);  // added by splautz
...
function rm($fileglob)
{
   if (is_string($fileglob)) {
       if (is_file($fileglob)) {
           ...
           if ($matching === false) return false;  // No files match supplied glob
   ...
   } else return false;  // Param #1 must be filename or glob pattern, or array of filenames or glob patterns

   return true;
}

// added by splautz for help info
  function tep_echo_help($info_tag) {
    global $languages_id;
    ...
    if ($info_help = tep_db_fetch_array($info_help_query)) {
      $info_text = isset($info_help['info_text'])?$info_help['info_text']:'';
    } else {
	  $info_text = '';
      $sql_data_array = array('info_tag' => tep_db_prepare_input($info_tag), 'language_id' => (int)$languages_id, 'info_text' => $info_text);
      tep_db_perform(TABLE_INFO_HELP, $sql_data_array);
    }
    if (strlen($info_text)) {
      $ret = '<script language="javascript">' . "\n<!--\n";
      $ret .= "document.write('";
      $ret .= '<a href="javascript:popupWindow(\\\'' . tep_href_link(FILENAME_INFO_HELP_POPUP,'info_tag='.$info_tag) . '\\\'' . ',' . (200) . ',' . (200) . ')" class="infoTag">' .  constant($info_tag) . '</a>' ."');";
      $ret .= "\n//--></script>\n";
      $ret .= "<noscript>\n";
      $ret .= '<a href="' . tep_href_link(FILENAME_INFO_HELP_POPUP,'info_tag='.$info_tag) . '" class="infoTag" target="_blank">' . constant($info_tag) . '</a>';
      $ret .= "\n</noscript>\n";
      echo $ret;
    } else echo constant($info_tag);
  }

  function tep_get_browser()
  {
