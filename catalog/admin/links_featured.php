...
*/
  ...
  $linkPartners = array();
  $linkPartners_query = tep_db_query("SELECT links_id, links_url FROM " . TABLE_LINKS . " WHERE links_status = '2'");
  while ($list = tep_db_fetch_array($linkPartners_query))
  {
    $linkPartners[] = array('id' => $list['links_id'], 'text' => $list['links_url']);
  }
  $linkFeatured = array();
  $linkFeatured_query = tep_db_query("SELECT lf.links_id, ld.links_id, ld.links_title FROM " . TABLE_LINKS_FEATURED . " lf, " . TABLE_LINKS_DESCRIPTION . " ld where lf.links_id = ld.links_id");
  while ($list = tep_db_fetch_array($linkFeatured_query))
  {
    $linkFeatured[] = array('id' => $list['links_id'], 'text' => $list['links_title']);
  }  
?>
       ...
       <tr><form name="new_feature" <?php echo 'action="' . tep_href_link(FILENAME_LINKS_FEATURED, tep_get_all_get_params(array('action', 'info', 'sID')) . 'action=process', 'NONSSL') . '"'; ?> method="post">
        <td><table border="0" cellspacing="0" cellpadding="2">
         ...
         <tr>
          ...
          <td><?php echo tep_draw_pull_down_menu('link_featured', $linkFeatured, '', '');?></td>
         </tr>                
         <tr>
          <td><?php echo tep_draw_separator('pixel_trans.gif', '100%', '10'); ?></td>
         </tr>         
