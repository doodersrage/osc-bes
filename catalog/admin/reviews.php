...
<?php
  if ($action == 'edit') {
    ...
    $products_name_query = tep_db_query("select products_name, products_img_alt from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . (int)$reviews['products_id'] . "' and language_id = '" . (int)$languages_id . "'");
      ...
      <tr><?php echo tep_draw_form('review', FILENAME_REVIEWS, 'page=' . $HTTP_GET_VARS['page'] . '&rID=' . $HTTP_GET_VARS['rID'] . '&action=preview'); ?>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            ...
            <td class="main" align="right" valign="top"><?php echo tep_image(HTTP_CATALOG_SERVER . DIR_WS_CATALOG_IMAGES . $rInfo->products_image, $rInfo->products_img_alt?$rInfo->products_img_alt:$rInfo->products_name, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'hspace="5" vspace="5"'); ?></td>
          </tr>
        </table></td>
      </tr>
      <tr>
...
<?php
    ...
    switch ($action) {
      ...
      if (isset($rInfo) && is_object($rInfo)) {
        ...
        $contents[] = array('text' => '<br>' . tep_info_image($rInfo->products_image, $rInfo->products_img_alt?$rInfo->products_img_alt:$rInfo->products_name, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT));
