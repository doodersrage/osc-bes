...
if (isset($stage) AND $stage=="payupdate") {
  pay_update($pay_menu,$paytype);
  echo MATCH_SUCCESS;
}
...
?>
<table class="lists">
<form action="<?php echo $_SERVER[PHP_SELF]?>" method="post" name="qbi_payment" id="qbi_payment">
<input name="stage" id="stage" type="hidden" value="payupdate" />
<input name="search_page" id="search_page" type="hidden" value="<?php echo $search_page?>" />
<?php
$Query = "SELECT COUNT(*) AS cnt FROM ".TABLE_QBI_PAYOSC." WHERE language_id='$languages_id'"; 
...
if($count > 0){ 
  ...
  $resultqbc = tep_db_query("SELECT * FROM ".TABLE_QBI_PAYOSC." WHERE language_id='$languages_id' ORDER BY qbi_payosc_text ".$limit);
  $hstring = $page->make_head_string(PAY_TITLE); 
  ...
  while ($myrowqbc = tep_db_fetch_array($resultqbc)) {
    echo "<tr><td class='oscmodel'>".$myrowqbc["qbi_payosc_text"]."</td>";
    pay_dropdown($myrowqbc["qbi_payosc_mod"]);
  }
echo "<tr><td colspan=\"2\">&nbsp;</td></tr>\r\n";
