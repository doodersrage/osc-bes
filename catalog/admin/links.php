...
// define our link functions
  ...
  for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
    $languages_array[] = array('id' => $i + 1, //$languages[$i]['id'],
                          ...
                          //     echo 'set '.$i .' '.$languages[$i]['id'].' '.$languages[$i]['name'].'<br>';
  }                           
  ...
  if (tep_not_null($action)) {  
    switch ($action) {
      ...
      case 'update':
        ...
        $links_language = tep_db_prepare_input($HTTP_POST_VARS['links_language']);
        ...
        if ($error == false) {
          ...
          if (isset($HTTP_POST_VARS['links_notify']) && ($HTTP_POST_VARS['links_notify'] == 'on')) {
            ...
            {
              if ($HTTP_POST_VARS['links_notify_denied_reason'] == '0')
                $statusUpdate =  $links_status_array[$links_status] . "\n\n\t" . 'Reason: RECIPROCAL link not found: ' .$links_reciprocal_url;
              else if ($HTTP_POST_VARS['links_notify_denied_reason'] == '1')
      ...
      case 'deleteconfirm':
        ...
        break;
      default:
        $links_query = tep_db_query("select l.links_id, ld.links_title, l.links_url, ld.language_id, ld.links_description, l.links_contact_email, l.links_status, l.links_image_url, l.links_contact_name, l.links_reciprocal_url, l.links_category_suggest, l.links_status, l.links_rating from " . TABLE_LINKS . " l left join " . TABLE_LINKS_DESCRIPTION . " ld on ld.links_id = l.links_id where ld.links_id = l.links_id and l.links_id = '" . (int)$HTTP_GET_VARS['lID'] ."'");
        $links = tep_db_fetch_array($links_query);

        $categories_query = tep_db_query("select lcd.link_categories_name as links_category from " . TABLE_LINKS_TO_LINK_CATEGORIES . " l2lc left join " . TABLE_LINK_CATEGORIES_DESCRIPTION . " lcd on lcd.link_categories_id = l2lc.link_categories_id where l2lc.links_id = '" . (int)$HTTP_GET_VARS['lID'] . "'");
        $category = tep_db_fetch_array($categories_query);
...
<?php
  if ($error == true) {
    if ($entry_links_title_error == true) {
      echo tep_draw_input_field('links_title', $lInfo->links_title, 'maxlength="64"') . '&nbsp;' . '<font color="red">' . ENTRY_LINKS_TITLE_ERROR . '</font>';
    } else {
...
<?php
  if ($error == true) {
    if ($entry_links_url_error == true) {
      echo tep_draw_input_field('links_url', $lInfo->links_url, 'maxlength="255"') . '&nbsp;' . '<font color="red">' . ENTRY_LINKS_URL_ERROR .'</font.>';
    } else {
...
<?php
  ...
  echo '   ' . ENTRY_LINKS_CATEGORY_SUGGEST . ' <b>'. $lInfo->links_category_suggest . ' </b>as a category for this link';
?>
           </td>
          </tr>
          <tr>
...
<?php
    if ($error == true) {
      if ($entry_links_contact_name_error == true) {
        echo tep_draw_input_field('links_contact_name', $lInfo->links_contact_name, 'maxlength="64"', true) . '&nbsp;' . '<font color="red">' . ENTRY_LINKS_CONTACT_NAME_ERROR . '</font?';
      } else {
...
<?php
  if ($error == true) {
    if ($entry_links_reciprocal_url_error == true) {
      echo tep_draw_input_field('links_reciprocal_url', $lInfo->links_reciprocal_url, 'maxlength="255"') . '&nbsp;' . '<font color="red">' . ENTRY_LINKS_RECIPROCAL_URL_ERROR . '</font>';
    } else {
...
?></td>
           ...
           <td><img src="<?php echo $img; ?>" alt="">
           <td width="20"</td>
           <td><a href="<?php echo $lInfo->links_reciprocal_url ?>" target="blank">Visit Reciprocal Link</a></td>
           <?php } ?>
          </tr>
        </table></td>
      </tr>
...
<?php
  if ($error == true) {
    if ($entry_links_rating_error == true) {
      echo tep_draw_input_field('links_rating', $lInfo->links_rating, 'size ="2" maxlength="2"') . '&nbsp;' . '<font color="red">' . ENTRY_LINKS_RATING_ERROR . '</font>';
    } else {
...
?>
    ...
    $links_query = tep_db_query("select l.links_id, ld.links_title, l.links_reciprocal_url from " . TABLE_LINKS . " l, " . TABLE_LINKS_DESCRIPTION . " ld where ld.links_id = l.links_id");
    ...
    while ($links = tep_db_fetch_array($links_query)) {  
      ...
      if ($link_check_status_text == TEXT_INFO_LINK_CHECK_FOUND) 
         $img = 'images/mark_check.jpg';
      else
         $img = 'images/mark_x.jpg';
