...
tep_debug("Search Rsult:");
  ...
  if (empty($dfrom) && empty($dto) && empty($pfrom) && empty($pto) && empty($keywords)&& empty($manufacturers_id)) {
    $error = true;
  ...
  if ($error == true) {
    tep_redirect(tep_mobile_link(FILENAME_ADVANCED_SEARCH, tep_get_all_get_params(), 'NONSSL', true, false));
  }

  $breadcrumb->add(NAVBAR_TITLE_1, tep_mobile_link(FILENAME_ADVANCED_SEARCH));
  $breadcrumb->add(NAVBAR_TITLE_2, tep_mobile_link(FILENAME_ADVANCED_SEARCH_RESULT, tep_get_all_get_params(), 'NONSSL', true, false));
...
$headerTitle->write(NAVBAR_TITLE_2);
