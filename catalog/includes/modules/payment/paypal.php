...
LISTEN UP!!!
I removed all instances of shipping in the cart => paypal transaction. What does this mean?
It means that PayPal will never list shipping in any order transactions. However, should
you use a coupon with free or discounted shipping in it, PayPal will always recieve the correct
charge amount.
...
// catalog payment module class methods
    ...
    function formFields($txn_sign = '', $payment_amount = '', $payment_currency = '', $payment_currency_value = '', $orders_id = '', $return_url = '', $cancel_url = '' ) {
      ...
      if(MODULE_PAYMENT_PAYPAL_METHOD == 'Itemized') {
        ...
        tep_draw_hidden_field('handling_cart', number_format(/*$order->info['shipping_cost']*/0 * $my_currency_value, $currencies->get_decimal_places($my_currency)));
        ...
        tep_draw_hidden_field('shipping', number_format(/*$order->info['shipping_cost']*/0 * $my_currency_value, $currencies->get_decimal_places($my_currency)));
        $item_number = '';
