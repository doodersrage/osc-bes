...
// class constructor
    function ups() {
      ...
      $this->enabled = ((MODULE_SHIPPING_UPS_STATUS == 'True') ? true : true);
...
// added by splautz for free shipping
      $this->free_types = preg_split("/\W+/",MODULE_SHIPPING_UPS_FREE,-1,PREG_SPLIT_NO_EMPTY);

    }

// class methods
    function quote($method = '') {
      global $HTTP_POST_VARS, $order, $shipping_weight_x, $shipping_num_boxes_x, $shipping_count_x;
...
//    $this->_upsOrigin(SHIPPING_ORIGIN_ZIP, $country_name['countries_iso_code_2']);
      $country_name = tep_get_countries(tep_get_location_code($orig_code, 1), true);
      $this->_upsOrigin(tep_get_location_code($orig_code, 2), $country_name['countries_iso_code_2']);
...
// modified by splautz for free shipping
      ...
      $this->_upsWeight($shipping_weight_x[0]);
      $uquote[0] = $this->_upsGetQuote();
      if (!is_array($uquote[0]) || isset($uquote[0]['error'])) $upsQuote = $uquote[0];
      else {
        if (!$shipping_count_x[1] || $shipping_weight_x[0] != $shipping_weight_x[1]) {
          if ($shipping_count_x[1]) {
      // get quote (1 => quote excluding free items)
            $this->_upsWeight($shipping_weight_x[100000]);
            $uquote[1] = $this->_upsGetQuote();
        ...
        foreach($uquote[0] as $urate) {
          ...
          elseif (in_array($qtype,$this->free_types) || !is_numeric(MODULE_SHIPPING_UPS_FREE_DISC))  // if still a free rate or limit undefined, reduce by max discount
            $qcost -= $max_discount;
          else // for all others reduce by max discount up to limit if appropriate
            $qcost -= min($max_discount,MODULE_SHIPPING_UPS_FREE_DISC);
          if ($qcost < 0) $qcost = 0;
          $upsQuote[] = array($qtype => $qcost);
        }
      }
      ...
      if ( (is_array($upsQuote)) && (sizeof($upsQuote) > 0) ) {
        ...
        for ($i=0; $i<$qsize; $i++) {
          ...
          $methods[] = array('id' => $type,
                             ...
                             'cost' => $cost + MODULE_SHIPPING_UPS_HANDLING * $shipping_num_boxes_x[1]);
        }
    ...
    function install() {
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Allow Free Shipping?', 'MODULE_SHIPPING_UPS_FREE', 'GND', 'Allow free shipping for these rates. GND - Ground, 3DS - 3 Day Select, 2DA - 2nd Day Air, 1DP - Next Day Air Saver, 1DA - Next Day Air, 1DM - Next Day Air Early AM', '6', '0', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Free Shipping Discount', 'MODULE_SHIPPING_UPS_FREE_DISC', '', 'All other rates will be reduced by up to this amount on free shipping items.  Leave blank to reduce by value of highest free rate.', '6', '0', now())");
    }
    ...
    function keys() {
      return array('MODULE_SHIPPING_UPS_STATUS', 'MODULE_SHIPPING_UPS_PICKUP', 'MODULE_SHIPPING_UPS_PACKAGE', 'MODULE_SHIPPING_UPS_RES', 'MODULE_SHIPPING_UPS_HANDLING', 'MODULE_SHIPPING_UPS_TAX_CLASS', 'MODULE_SHIPPING_UPS_ZONE', 'MODULE_SHIPPING_UPS_SORT_ORDER', 'MODULE_SHIPPING_UPS_FREE', 'MODULE_SHIPPING_UPS_FREE_DISC');
    }
    ...
    function _upsWeight($foo) {
      $this->_upsPackageWeight = ($foo<1)?1:$foo;  // modified by splautz to avoid 0 weights
    }
