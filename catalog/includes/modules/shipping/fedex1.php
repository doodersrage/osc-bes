...
// added by splautz for free shipping
      $this->free_types = preg_split("/\W+/",MODULE_SHIPPING_FEDEX1_FREE,-1,PREG_SPLIT_NO_EMPTY);

    }

// class methods
    function quote($method = '') {
      global $shipping_weight_x, $shipping_num_boxes_x, $shipping_count_x, $cart, $order;
      ...
      if (MODULE_SHIPPING_FEDEX1_ENVELOPE == 'True' && $shipping_weight_x[0]) {  // modified by splautz to avoid 0 weights
        if ( ($shipping_weight_x[0] <= .5 && MODULE_SHIPPING_FEDEX1_WEIGHT == 'LBS') ||
             ($shipping_weight_x[0] <= .2 && MODULE_SHIPPING_FEDEX1_WEIGHT == 'KGS')) {
      ...
      $this->_setWeight($shipping_weight_x[0]);

      $totals = $cart->show_total();
      $this->_setInsuranceValue($totals / $shipping_num_boxes_x[0]);
...
//      $countries_array = tep_get_countries(SHIPPING_ORIGIN_COUNTRY, true);
        $countries_array = tep_get_countries(tep_get_location_code($orig_code, 1), true);
...
// modified by splautz for free shipping
      ...
      // get quote (1 => quote excluding free items)
          ...
          // find max discount
          foreach($uquote[0] as $qtype => $qcost) {
        ...
        foreach($uquote[0] as $qtype => $qcost) {
          ...
          elseif (in_array($qtype,$this->free_types) || !is_numeric(MODULE_SHIPPING_FEDEX1_FREE_DISC))  // if still a free rate or limit undefined, reduce by max discount
            $qcost -= $max_discount;
          else // for all others reduce by max discount up to limit if appropriate
            $qcost -= min($max_discount,MODULE_SHIPPING_FEDEX1_FREE_DISC);
          if ($qcost < 0) $qcost = 0;
          $fedexQuote[$type] = $qcost;
        }
      }
      ...
      if (is_array($fedexQuote)) {
        if (isset($fedexQuote['error'])) {
          ...
          $this->quotes = array('id' => $this->code,
                                'module' => $this->title . ' (' . $shipping_num_boxes_x[0] . ' x ' . $shipping_weight_x[0] . strtolower(MODULE_SHIPPING_FEDEX1_WEIGHT) . ')');
          ...
          foreach ($fedexQuote as $type => $cost) {
            ...
            if (!$skip) {
              $methods[] = array('id' => substr($type,0,2),
                                 'title' => $service_descr,
                                 'cost' => $cost + (SHIPPING_HANDLING + MODULE_SHIPPING_FEDEX1_SURCHARGE + $this->surcharge) * $shipping_num_boxes_x[1]);
            }
          }
    ...
    function install() {
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Allow Free Shipping?', 'MODULE_SHIPPING_FEDEX1_FREE', '92 90', 'Allow free shipping for these rates. 92 - Ground, 90 - Home Delivery (Res Gnd), 20 - Express Saver, 06 - First Overnight, 05 - Standard Overnight, 03 - 2 Day Air, 01 - Priority', '6', '0', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Free Shipping Discount', 'MODULE_SHIPPING_FEDEX1_FREE_DISC', '', 'All other rates will be reduced by up to this amount on free shipping items.  Leave blank to reduce by value of highest free rate.', '6', '0', now())");
    }
...
// added by splautz to avoid 0 weights
      if ($this->packageType == '01') {
        if ($pounds < 1) $pounds = 1;
      } elseif ($pounds < 0.1) $pounds = 0.1;
...
//    $data .= '8,"' . MODULE_SHIPPING_FEDEX1_STATE . '"'; // Sender state code
      $data .= '8,"' . tep_get_location_code($orig_code, 3) . '"'; // Sender state code
//    $orig_zip = str_replace(array(' ', '-'), '', MODULE_SHIPPING_FEDEX1_POSTAL);
      $orig_zip = str_replace(array(' ', '-'), '', tep_get_location_code($orig_code, 2));
