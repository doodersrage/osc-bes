...
// class methods
    function quote($method = '') {
      ...
      if ($dest_zone == 0) {
        ...
        for ($i=0; $i<$size; $i+=2) {
          if ($shipping_weight_x[1] <= $zones_table[$i]) {
            $shipping = $zones_table[$i+1];
            $shipping_method = MODULE_SHIPPING_ZONES_TEXT_WAY . ' ' . $dest_country . ' : ' . $shipping_weight_x[0] . ' ' . MODULE_SHIPPING_ZONES_TEXT_UNITS;
            break;
          }
        }
        ...
        if ($shipping == -1) {
          ...
          $shipping_cost = ($shipping * $shipping_num_boxes_x[1]) + ($shipping_num_boxes_x[1]?constant('MODULE_SHIPPING_ZONES_HANDLING_' . $dest_zone):0);  // modified by splautz for free shipping
        }
      }
