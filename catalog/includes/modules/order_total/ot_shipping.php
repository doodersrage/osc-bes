...
//        $order->info['shipping_cost'] = 0;
          $order->info['shipping_cost'] -= $order->info['shipping_cost_0'];
          $order->info['total'] -= $order->info['shipping_cost_0'];
          $order->info['shipping_cost_0'] = 0;
...
//    if (tep_not_null($order->info['shipping_method'])) {
      if (tep_not_null($order->info['shipping_method'])) {
        $module = substr($GLOBALS['shipping'][0]['id'], 0, strpos($GLOBALS['shipping'][0]['id'], '_'));
...
/* modified by splautz for freight shipping
        ...
        $this->output[] = array('title' => $order->info['shipping_method'] . ':',
          ...
          $order->info['tax'] += tep_calculate_tax($order->info['shipping_cost_0'], $shipping_tax);
          $order->info['tax_groups']["$shipping_tax_description"] += tep_calculate_tax($order->info['shipping_cost_0'], $shipping_tax);
          $order->info['total'] += tep_calculate_tax($order->info['shipping_cost_0'], $shipping_tax);

          if (DISPLAY_PRICE_WITH_TAX == 'true') {
            $order->info['shipping_cost_0'] += tep_calculate_tax($order->info['shipping_cost_0'], $shipping_tax);
            $order->info['shipping_cost'] += tep_calculate_tax($order->info['shipping_cost_0'], $shipping_tax);
          }
        }

        $this->output[] = array('title' => $order->info['shipping_method'] . ':',
                                'text' => $currencies->format($order->info['shipping_cost_0'], true, $order->info['currency'], $order->info['currency_value']),
                                'value' => $order->info['shipping_cost_0']);
    ...
    function install() {
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Display Shipping', 'MODULE_ORDER_TOTAL_SHIPPING_STATUS', 'true', 'Do you want to display the order shipping cost (non-freight)?', '6', '1','tep_cfg_select_option(array(\'true\', \'false\'), ', now())");
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Allow Free Shipping', 'MODULE_ORDER_TOTAL_SHIPPING_FREE_SHIPPING', 'false', 'Do you want to allow free shipping for non-freight items?', '6', '3', 'tep_cfg_select_option(array(\'true\', \'false\'), ', now())");
