...
// added by splautz for freight shipping
      $freight_method_query = tep_db_query("select title, value from " . TABLE_ORDERS_TOTAL . " where orders_id = '" . (int)$order_id . "' and class = 'ot_freight'");
      $freight_method = tep_db_fetch_array($freight_method_query);
...
//begin PayPal_Shopping_Cart_IPN
                          ...
                          'shipping_cost' => $shipping_method['value']+$freight_method['value'],  // modified by splautz for freight shipping
                          'shipping_cost_0' => $shipping_method['value'],  // added by splautz for freight shipping
                          'shipping_cost_1' => $freight_method['value'],  // added by splautz for freight shipping
...
// added by splautz for freight shipping
                          'freight_method' => ((substr($freight_method['title'], -1) == ':') ? substr(strip_tags($freight_method['title']), 0, -1) : strip_tags($freight_method['title'])));
...
//                        'shipping_cost' => $shipping['cost'],
                          'shipping_method' => $shipping[0]['title'],
                          'freight_method' => $shipping[1]['title'],
                          'shipping_cost' => $shipping[0]['cost'] + $shipping[1]['cost'],
                          'shipping_cost_0' => $shipping[0]['cost'],
                          'shipping_cost_1' => $shipping[1]['cost'],
...
//                                      'final_price' => $products[$i]['price'] + $cart->attributes_price($products[$i]['id']),
                                        'final_price' => $products[$i]['final_price'],  // AAP fix
