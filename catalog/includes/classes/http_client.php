 ...
 * @seeAlso connect
 **/
    function httpClient($host = '', $port = '', $connection = 'NONSSL') {
 ...
 **/
    function Connect($host, $port = '', $connection = 'NONSSL') {
      // modified by splautz to support ssl
      $this->url['scheme'] = $connection=='SSL'?'https':'http';
 ...
 **/
    function Get($url, $processReply = true) {
      $this->responseHeaders = $this->responseBody = '';
 ...
 **/
    function Post($uri, $query_params = '', $processReply = true) {
      $uri = $this->makeUri($uri);
 ...
 **/
    function Put($uri, $filecontent, $processReply = true) {
      $uri = $this->makeUri($uri);
...
// connect if necessary
      if ( ($this->socket == false) || (feof($this->socket)) ) {
        if ($this->useProxy) {
          ...
          // modified by splautz to support ssl
          $host = ($this->url['scheme']=='https'?'ssl://':'').$this->url['host'];
          $port = !empty($this->url['port'])?$this->url['port']:($this->url['scheme']=='https'?443:80);
        }
