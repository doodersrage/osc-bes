...
*/
  ...
  if (isset($HTTP_GET_VARS['products_id'])) {
    $manufacturer_query = tep_db_query("select m.manufacturers_id, m.manufacturers_name, m.manufacturers_image, mi.manufacturers_url, mi.manufacturers_img_alt from " . TABLE_MANUFACTURERS . " m left join " . TABLE_MANUFACTURERS_INFO . " mi on (m.manufacturers_id = mi.manufacturers_id and mi.languages_id = '" . (int)$languages_id . "'), " . TABLE_PRODUCTS . " p  where p.products_id = '" . (int)$HTTP_GET_VARS['products_id'] . "' and p.manufacturers_id = m.manufacturers_id and m.manufacturers_status = '1'");  // modified by splautz for status
...
<?php
      ...
      if (tep_not_null($manufacturer['manufacturers_image'])) $manufacturer_info_string .= '<tr><td align="center" class="infoBoxContents" colspan="2">' . tep_image(DIR_WS_IMAGES . $manufacturer['manufacturers_image'], $manufacturer['manufacturers_img_alt']?$manufacturer['manufacturers_img_alt']:$manufacturer['manufacturers_name']) . '</td></tr>';
      else $manufacturer_info_string .= '<tr><td valign="top" class="infoBoxContents" colspan="2"><b>' .  $manufacturer['manufacturers_name'] . '</b></td></tr>';
