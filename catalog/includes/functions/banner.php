...
// modified by splautz to support homepage-only banners & no banners on SSL
  function tep_display_banner($action, $identifier) {
// added to support homepage-only banners & no banners on SSL
  	global $PHP_SELF, $HTTP_GET_VARS, $request_type;
    if ($request_type == 'SSL')
      return '<b>TEP ERROR! (tep_display_banner(' . $action . ', ' . $identifier . ') -> No banners should be displayed on SSL pages</b>';
    if (basename($PHP_SELF) != FILENAME_DEFAULT || isset($HTTP_GET_VARS['cPath'])) {
      $homepage_query = " and homepage_only = '0'";
    } else $homepage_query = '';

    if ($action == 'dynamic') {
      $banners_query = tep_db_query("select count(*) as count from " . TABLE_BANNERS . " where status = '1'" . $homepage_query . " and banners_group = '" . $identifier . "'");
      $banners = tep_db_fetch_array($banners_query);
      if ($banners['count'] > 0) {
        $banner = tep_random_select("select banners_id, banners_title, banners_image, banners_html_text from " . TABLE_BANNERS . " where status = '1'" . $homepage_query . " and banners_group = '" . $identifier . "'");
      } else {
      ...
      if (is_array($identifier)) {
        ...
        $banner_query = tep_db_query("select banners_id, banners_title, banners_image, banners_html_text from " . TABLE_BANNERS . " where status = '1'" . $homepage_query . " and banners_id = '" . (int)$identifier . "'");
...
// modified by splautz to support displaying of mutiple banners, sort levels, homepage-only, & no banners on SSL
// Use 'dynamic' to return a random banner of the group $identifier
// Use $count with 'dynamic' to return an array of $count banners, properly sorted, or randomized if same sort_order
// Use 'static' to return the banner of the banner_id given in $identifier
  function tep_banner_exists($action, $identifier, $count = NULL) {
// added to support homepage-only banners & no banners on SSL
  	global $PHP_SELF, $HTTP_GET_VARS, $request_type;
    if ($request_type == 'SSL') return false;
    if (basename($PHP_SELF) != FILENAME_DEFAULT || isset($HTTP_GET_VARS['cPath'])) {
      $homepage_query = " and homepage_only = '0'";
    } else $homepage_query = '';
...
// get sorted banners
        $banners_array = array();
        $banners_query = tep_db_query("select banners_id, banners_title, banners_image, banners_html_text from " . TABLE_BANNERS . " where status = '1'" . $homepage_query . " and banners_group = '" . $identifier . "' order by COALESCE(sort_order,1000), rand() limit $count");
        while ($banners = tep_db_fetch_array($banners_query)) $banners_array[] = $banners;
        return (count($banners_array)?$banners_array:false);

      } else return tep_random_select("select banners_id, banners_title, banners_image, banners_html_text from " . TABLE_BANNERS . " where status = '1'" . $homepage_query . " and banners_group = '" . $identifier . "'");
    } elseif ($action == 'static') {
      $banner_query = tep_db_query("select banners_id, banners_title, banners_image, banners_html_text from " . TABLE_BANNERS . " where status = '1'" . $homepage_query . " and banners_id = '" . (int)$identifier . "'");
