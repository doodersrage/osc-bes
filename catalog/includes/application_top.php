...
//  }
  if (function_exists('ini_get')) {
    !ini_get('register_globals') or exit('Server Requirement Error: register_globals is enabled in your PHP configuration. This can be disabled in your php.ini configuration file or in the .htaccess file in your catalog directory.');
  }
...
// set the language
// modified by splautz to only reset language if it is different & for header links
// if (!tep_session_is_registered('language') || isset($HTTP_GET_VARS['language'])) {
  include(DIR_WS_CLASSES . 'language.php');
  $lng = new language();
  if (!tep_session_is_registered('language') || (isset($HTTP_GET_VARS['language']) && $HTTP_GET_VARS['language'] != $language && $HTTP_GET_VARS['language'] != $languages_id)) {
...
// modified by splautz for header links
//}
  } else {
    $lng->set_language($languages_id);
  }
  $lng->alt_languages();
  ...
  if (isset($HTTP_POST_VARS['wishlist_x'])) {
    if (isset($HTTP_POST_VARS['products_id'])) {
      if (!is_array($HTTP_POST_VARS['products_id'])) {
        $HTTP_POST_VARS['products_id'] = array($HTTP_POST_VARS['products_id']);
        $HTTP_POST_VARS['id'] = array($HTTP_POST_VARS['id']);
        $HTTP_POST_VARS['cart_quantity'] = isset($HTTP_POST_VARS['cart_quantity']) ? array($HTTP_POST_VARS['cart_quantity']) : array(1);
      }
      for ($i=0, $n=sizeof($HTTP_POST_VARS['products_id']); $i<$n; $i++) 
        if (is_numeric($HTTP_POST_VARS['products_id'][$i]) && $HTTP_POST_VARS['cart_quantity'][$i] > 0) {
          $wishList->add_wishlist($HTTP_POST_VARS['products_id'][$i], $HTTP_POST_VARS['id'][$i]);
          $wishlist_updated = true;
        }
    }
    if(DISPLAY_WISHLIST == 'true') tep_redirect(tep_href_link(FILENAME_WISHLIST));
// Shopping cart actions
  } elseif (isset($HTTP_GET_VARS['action']) && basename($PHP_SELF) != FILENAME_REDIRECT) { // modified by splautz to not check redirect actions
// added by splautz for affiliate linking
    if ($HTTP_GET_VARS['action'] == 'buy_now_aff' && $HTTP_GET_VARS['products_id']) {
      tep_redirect(tep_href_link(FILENAME_REDIRECT, 'action=aff_link&products_id=' . $HTTP_GET_VARS['products_id'], 'NONSSL', false, false));
	}
// redirect the customer to a friendly cookie-must-be-enabled page if cookies are disabled
    ...
    switch ($HTTP_GET_VARS['action']) {
      ...
      // modified by splautz to support adding multiple products
      case 'add_product' :    if (isset($HTTP_POST_VARS['products_id']))
                                ...
                                for ($i=0, $n=sizeof($HTTP_POST_VARS['products_id']); $i<$n; $i++) 
                                  if (is_numeric($HTTP_POST_VARS['products_id'][$i]) && $HTTP_POST_VARS['cart_quantity'][$i] > 0)
                                    $cart->add_cart($HTTP_POST_VARS['products_id'][$i], $cart->get_quantity(tep_get_uprid($HTTP_POST_VARS['products_id'][$i], $HTTP_POST_VARS['id'][$i]))+$HTTP_POST_VARS['cart_quantity'][$i], $HTTP_POST_VARS['id'][$i]);
                              tep_redirect(tep_href_link($goto, tep_get_all_get_params($parameters)));
                              break;
...
// added by splautz to ensure $cPath is complete
  if (isset($cPath_array[0]) && $cPath_array[0])
    do {
      $categories_query = tep_db_query("select c.parent_id from " . TABLE_CATEGORIES . " c where c.categories_id = '" . strval($cPath_array[0]) . "'");
      $categories = tep_db_fetch_array($categories_query);
      $pid = 0;
      if (tep_db_num_rows($categories_query) > 0 && $pid=$categories['parent_id']) {
        array_unshift($cPath_array, $pid);
        $cPath = strval($pid).'_'.$cPath;
      }
    } while ($pid);
...
// changed by splautz to eliminate "catalog" in breadcrumb
...
//################## End Enable / Disable Categories #################
      if (tep_db_num_rows($categories_query) > 0) {
        ...
        header("HTTP/1.0 404 Not Found"); exit;  // added by splautz to 404 on category not found
        break;
      }
    }
  } elseif (isset($HTTP_GET_VARS['manufacturers_id']) && is_numeric($HTTP_GET_VARS['manufacturers_id'])) {
    $manufacturers_query = tep_db_query("select manufacturers_name from " . TABLE_MANUFACTURERS . " where manufacturers_id = '" . (int)$HTTP_GET_VARS['manufacturers_id'] . "'");
    if (tep_db_num_rows($manufacturers_query)) {
      $manufacturers = tep_db_fetch_array($manufacturers_query);
      $breadcrumb->add($manufacturers['manufacturers_name'], tep_href_link(FILENAME_DEFAULT, 'manufacturers_id=' . $HTTP_GET_VARS['manufacturers_id']));
    }
// added by splautz to 404 on manufacturer not found
    else {
...
// added by splautz to 404 on product not found
    else {
      header("HTTP/1.0 404 Not Found"); exit;
    }
...
// Wishlist
  if ($wishlist_updated) $messageStack->add('header', PRODUCT_ADDED_TO_WISHLIST, 'success');
