...
*******************************************************************/
                  ...
                  // Now get and populate product attributes
                    $wishlist_products_attributes_query = tep_db_query("select products_options_id as po, products_options_value_id as pov, products_options_value_text as pot from " . TABLE_WISHLIST_ATTRIBUTES . " where customers_id='" . $public_id . "' and products_id = '" . tep_db_input($wishlist['products_id']) . "'");
                    while ($wishlist_products_attributes = tep_db_fetch_array($wishlist_products_attributes_query)) {
                      ...
                      $attributes = tep_db_query("select popt.products_options_name, poval.products_options_values_name, pa.options_values_price, pa.price_prefix
                       ...
                       if ($wishlist_products_attributes['pov'] == PRODUCTS_OPTIONS_VALUE_TEXT_ID) {
                         echo tep_draw_hidden_field('id['.$wishlist['products_id'].']['.TEXT_PREFIX.$wishlist_products_attributes['po'].']', $wishlist_products_attributes['pot']);
                         $attr_value = $wishlist_products_attributes['pot'];
                       } else {
                         echo tep_draw_hidden_field('id['.$wishlist['products_id'].']['.$wishlist_products_attributes['po'].']', $wishlist_products_attributes['pov']);
                       ...
                       if ($attributes_values['price_prefix'] == '+') {
                         ...
                         $attributes_addon_price -= $attributes_values['options_values_price'];
                       } else $attributes_addon_price += $attributes_values['options_values_price'] - $products['final_price'];
                       echo '<br /><small><i> ' . $attributes_values['products_options_name'] . ': ' . $attr_value . '</i></small>';
