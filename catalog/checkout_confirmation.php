...
// multiple orders
  if(tep_not_null($HTTP_POST_VARS['delete'])){    
  //some validation to prevent orders being delete from other customer accounts and already paid orders
	$order_delete_query = tep_db_query("select orders_id from " . TABLE_ORDERS . " where orders_id = '" . (int)$HTTP_POST_VARS['delete'] . "' and customers_id = '" . (int)$customer_id . "' and paid_order = '0'");
...
// paypal wpp
if(tep_session_is_registered('pp_payer_id')) {   $orderTotal = $order->info[total]; $changeShippingInfo = "paypal_wpp/ec/process.php?express=1&amount=$orderTotal";  } 
else { $changeShippingInfo = FILENAME_CHECKOUT_SHIPPING_ADDRESS; }
...
?>
            <td width="30%" valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr>
                <td class="main"><?php echo '<b>' . HEADING_DELIVERY_ADDRESS . '</b> <a href="' . tep_href_link($changeShippingInfo, '', 'SSL') . '"><span class="orderEdit">(' . TEXT_EDIT . ')</span></a>'; ?></td>
              </tr>
              <tr>
                <td class="main"><?php echo tep_address_format($order->delivery['format_id'], $order->delivery, 1, ' ', '<br>'); ?></td>
              </tr>
<?php
    if ($order->info['shipping_method'] || $order->info['freight_method']) {  // modified by splautz to include displaying freight shipping
?>
              <tr>
...
<?php
      ...
      if ($order->info['freight_method']) {
?>
              <tr>
                <td class="main"><?php echo $order->info['freight_method']; ?></td>
              </tr>
<?php
      }
    }
        ...
        <td><table border="0" width="100%" cellspacing="1" cellpadding="2" class="infoBox">
          <tr class="infoBoxContents">
            <td width="30%" valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
              ...
              <tr>
                <td class="main"><?php echo '<b>' . HEADING_PAYMENT_METHOD . '</b> <a href="' . tep_href_link(FILENAME_CHECKOUT_PAYMENT . "?reset", '', 'SSL') . '"><span class="orderEdit">(' . TEXT_EDIT . ')</span></a>'; ?></td>              </tr>
              <tr>
...
//    if ($confirmation = $payment_modules->confirmation()) {
  ...
  if ($orders_total > 0) {
    $history_query = tep_db_query("select o.orders_id, o.date_purchased, o.delivery_name, o.billing_name, ot.text as order_total, s.orders_status_name from " . TABLE_ORDERS . " o, " . TABLE_ORDERS_TOTAL . " ot, " . TABLE_ORDERS_STATUS . " s where o.customers_id = '" . (int)$customer_id . "' and paid_order = '0' and o.orders_id = ot.orders_id and ot.class = 'ot_total' and o.orders_status = s.orders_status_id and s.language_id = '" . (int)$languages_id . "' order by orders_id DESC");
    $orders_to_pay = tep_db_num_rows($history_query); 
  }
...
?>
            <table width="100%" border="0" cellspacing="0" cellpadding="2">
              <tr>
                <td class="main" colspan="4"><?php echo TEXT_REMAINING_ORDERS; ?></td>
              </tr>
              <tr><td>
<?php
          ...
          <table border="0" width="100%" cellspacing="1" cellpadding="2">
            <tr>
              <td><table border="0" width="100%" cellspacing="2" cellpadding="4">
                <tr>
                  <td class="main" width="50%" valign="top"><b><?php echo TEXT_ORDER_NUMBER; ?></b> <?php echo $history['orders_id']; ?><br><b><?php echo TEXT_ORDER_DATE; ?></b> <?php echo tep_date_long($history['date_purchased']); ?><br><b><?php echo $order_type; ?></b> <?php echo tep_output_string_protected($order_name); ?></td>
                  <td class="main" width="30%" valign="top"><b><?php echo TEXT_ORDER_STATUS; ?></b> <?php echo $history['orders_status_name']; ?><br><b><?php echo TEXT_ORDER_PRODUCTS; ?></b> <?php echo $products['count']; ?><br><b><?php echo TEXT_ORDER_COST; ?></b> <?php echo strip_tags($history['order_total']); ?></td>
                  <td class="main" width="40%"><a href="<?php echo tep_href_link(FILENAME_ACCOUNT_HISTORY_INFO, 'order_id=' . $history['orders_id'], 'SSL'); ?>" target="_blank"><?php echo tep_image_button('small_view.gif', SMALL_IMAGE_BUTTON_VIEW); ?></a>  <?php
          echo tep_draw_form('delete_order' . $history['orders_id'], $form_action_url, 'post');
          reset($HTTP_POST_VARS);
          while (list($key, $value) = each($HTTP_POST_VARS)) {
            if (!is_array($HTTP_POST_VARS[$key]) && $key != 'delete') {
              echo tep_draw_hidden_field($key, htmlspecialchars(tep_db_prepare_input($value)));
            }
          }
                  ?><?php echo tep_draw_hidden_field('delete', $history['orders_id']); ?><?php echo tep_image_submit('small_delete.gif', SMALL_IMAGE_BUTTON_DELETE); ?></form></td>
                </tr>
              </table></td>
            </tr>
          </table>
