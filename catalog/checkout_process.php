...
// multiple orders
  if ($order->info['payment_method'] == MODULE_PAYMENT_MULTIPLE_ORDERS_TEXT_TITLE){
    ...
    $remaining_query = tep_db_query("select orders_id from " . TABLE_ORDERS . " where customers_id = '" . $customer_id . "' and paid_order = '0'");
    while ($remaining = tep_db_fetch_array($remaining_query)) {
      $sql_data_array = array('orders_id' => $remaining['orders_id'], 
    ...
    tep_db_query("update " . TABLE_ORDERS . " set payment_method = '". TEXT_PAID_WITH . " " . $insert_id . "', last_modified = now(), orders_status = '" . $order->info['order_status'] . "', paid_order = '1' where customers_id = '" . $customer_id . "' and paid_order = '0'");
  }
...
// update-20051113
          $attributes_query = "select popt.products_options_name, poval.products_options_values_name, pa.options_values_price, pa.price_prefix, pad.products_attributes_maxdays, pad.products_attributes_maxcount , pad.products_attributes_filename 
                               ...
                               left join " . TABLE_PRODUCTS_ATTRIBUTES_DOWNLOAD . " pad 
                                on pa.products_attributes_id = pad.products_attributes_id, " . TABLE_PRODUCTS_OPTIONS . " popt, " . TABLE_PRODUCTS_OPTIONS_VALUES . " poval 
        ...
        //clr 030714 update insert query.  changing to use values from $order->products for products_options_values.
        $sql_data_array = array('orders_id' => $insert_id, 
...
// moved by splautz for multiple orders
// $cart->reset(true);
...
// added by splautz for multiple orders
  if ($order->info['payment_method'] == MODULE_PAYMENT_MULTIPLE_ORDERS_TEXT_TITLE) {
    $messageStack->add_session('header', TEXT_MULTIPLE_ORDER_HEADER_SUCCESS, 'success');
    tep_redirect(tep_href_link(FILENAME_SHOPPING_CART), '', 'SSL');
  }
  $cart->reset(true);  // moved here to keep cart intact in case customer wants a duplicate order
