...
// paypal wpp
//if customer  chooses to change the payment method, unregister express checkout variable and make all payment options available. 
if(eregi("reset",$HTTP_SERVER_VARS[QUERY_STRING])) { 

//unregister session variable
...
<?php
  ...
  if (!tep_session_is_registered('payment')) $payment = $selection[0]['id']; // added by splautz to select first as default
...
<?php
    if (sizeof($selection) > 1) {
      ...
      echo tep_draw_hidden_field('payment', $selection[$i]['id'],'checked');
    }
?>
                    </td>
                    <td width="10"><?php echo tep_draw_separator('pixel_trans.gif', '10', '1'); ?></td>
...
//session is not registered continue with normal processing 
else {
// purchaseorders_1_4 start
  // If the customer is not allowed to enter a purchase order, then do not
	// display it.
  if($cust_allow_po_entry['customers_allow_purchase_order_entry'] != 'true')
  {
